...
/**
 * Finds the most recent Feed item from library items
 * Looking for items with "Feed" in the lessonName
 ...
 */
export function findFeedLabelingGuide(
  items: LibraryItem[] | undefined,
): LibraryItem | undefined {
  if (!items) return undefined

  // Filter for items with "Feed" keyword, extract year if present
  const itemsWithYear: ItemWithYear[] = []
  const itemsWithoutYear: LibraryItem[] = []

  items.forEach((item) => {
    const hasFeed = /feed/i.test(item.lessonName)
...
export function useFeedLabelingGuide(
  libraryItems: LibraryItem[] | undefined,
): LibraryItem | undefined {
  const [feed, setFeed] = useState<LibraryItem | undefined>(undefined)

  useEffect(() => {
    setFeed(findFeedLabelingGuide(libraryItems ?? undefined))
  }, [libraryItems])
