...
const fieldStyles = tv({
  slots: {
    ...
    label:
      'select-none text-base/6 text-site-fg group-disabled:opacity-50 sm:text-sm/6',
    fieldError:
  ...
  return (
    <Group
      ...
      className={composeTailwindRenderProps(className, [
        ...
        'inset-ring inset-ring-input shadow-xs group relative flex items-center overflow-hidden rounded-lg transition duration-200 ease-out',
        '[&>[role=progressbar]:first-child]:ml-(--gutter-x) [&>[role=progressbar]:last-child]:mr-(--gutter-x)',
        '**:data-[slot=icon]:size-4 **:data-[slot=icon]:shrink-0 **:[button]:shrink-0 *:data-[slot=icon]:z-10',
  ...
  return (
    <InputPrimitive
      ...
      className={composeTailwindRenderProps(
        className,
        'placeholder-muted-fg outline-hidden relative block w-full px-3.5 py-2 sm:px-3 sm:py-1.5 sm:text-sm/6 [&::-ms-reveal]:hidden [&::-webkit-search-cancel-button]:hidden',
      )}
    />
  )
}
