...
import { usePersonCreateMutation } from './usePersonCreateMutation'
...
describe('usePersonCreateMutation', () => {
  ...
  it('returns the response from a Successfully mutation', async () => {
    ...
    const {
      result: { current },
    } = renderHook(() => usePersonCreateMutation({ onSuccess, onError }), {
      wrapper,
    })

    act(() => {
      current.mutate({
        primaryElementID: 2184472,
        roleID: 503,
        logonName: 'tester@kv.com',
      })
    })
