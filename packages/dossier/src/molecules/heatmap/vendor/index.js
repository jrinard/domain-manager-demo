...
import { DEFAULT_GRADIENTS } from '../utils'
  ...
  var heatmapFactory = (function () {
    ...
    store.prototype = {
      ...
      addDataPoint: function (x, y) {
        ...
        if (me.max < data[x][y]) {
          ...
          heatmap
            .get('actx')
            .clearRect(0, 0, heatmap.get('width'), heatmap.get('height'))
    ...
    legend.prototype = {
      init: function () {
        ...
        element.style.cssText =
          ...
          positionCss +
          'font-family:Helvetica; width:256px;z-index:50; background:rgba(255,255,255,1);padding:10px;border:1px solid black;margin:0;'
        element.innerHTML =
          "<h3 style='padding:0;margin:0;text-align:center;font-size:16px;'>" +
          title +
          '</h3>'
        // create gradient in canvas
        labelsEl.style.cssText =
          'position:relative;font-size:12px;display:block;list-style:none;list-style-type:none;margin:0;height:15px;'

        // create gradient element
        gradient = document.createElement('div')
        gradient.style.cssText = [
          'position:relative;display:block;width:256px;height:15px;border-bottom:1px solid black; background-image:url(',
          me.createGradientImage(),
          ');',
        ].join('')
      ...
      update: function (max) {
        ...
        for (var i = 0; i < gradient.length; i++) {
          ...
          labelsHtml +=
            '<li style="position:absolute;left:' +
            ((((1 / (gradient.length - 1)) * i * 256 || 0) >> 0) -
              offset +
              0.5) +
            'px">' +
            labelText +
    ...
    heatmap.prototype = {
      configure: function (config) {
        ...
        me.set('gradient', config.gradient || { ...DEFAULT_GRADIENTS }) // default is the common blue to red gradient
      ...
      init: function () {
        ...
        // * Remove previous canvas' that are just wasted DOM nodes now
        const elem = me.get('element')
        const curCanvasItems = elem
          ? Array.from(elem.querySelectorAll('canvas'))
          : []

        if (curCanvasItems && curCanvasItems.length) {
          curCanvasItems.forEach((item, curIdx) => {
            if (curIdx < curCanvasItems.length - 1 && item && item.remove) {
              item.remove()
            }
          })
        }
      ...
      initColorPalette: function () {
        ...
        me.set(
          'premultiplyAlpha',
          testData.data[0] < 60 || testData.data[0] > 70,
        )
