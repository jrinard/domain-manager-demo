...
import { useMasteryConfigs } from '../../../data/hooks/useEditableUIConfig'
import {
  SidebarNavigation,
  NavigationSection,
...
export const DetailsDomainMastery = () => {
  ...
  } = useMasteryConfig({ configID: configID ?? '' })

  // Use editable state to track sections in real-time (updates when masteryConfig changes)
  const editable = useEditableMasteryConfig(
    masteryConfig ?? DEFAULT_CONFIGS.MASTERY_CONFIG,
  )
  ...
  const targetConfig = useMemo(() => {
    return masteryConfigsQuery.all_configs?.find(
      (c) => c.UIconfigGUID === configID,
    )
  }, [masteryConfigsQuery.all_configs, configID])
  ...
  // Check for changes whenever singleData changes
  useEffect(() => {
  ...
  const hasMetReq = (sectionId: string): boolean => {
    switch (sectionId) {
      ...
      case 'layout': {
        ...
        // Use editable.config.sections which updates when masteryConfig changes
    ...
    <div className="bg-site-bg flex h-full w-full flex-col">
      ...
      <div className="border-grayscale-600 bg-site-bg flex w-full flex-row justify-between border-b">
        <nav className="mt-2 flex items-center gap-2 px-2 py-2 text-sm text-gray-300">
          ...
          <Link
            to={`${DOMAIN_MANAGER_PATHS.BASE_ROUTE}/domain/${domainID}/mastery`}
