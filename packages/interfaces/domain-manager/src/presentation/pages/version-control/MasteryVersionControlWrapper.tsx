...
interface MasteryVersionControlWrapperProps {
  domainID?: number
  teamID?: number
}

export const MasteryVersionControlWrapper = ({
  domainID,
  teamID,
}: MasteryVersionControlWrapperProps) => {
  ...
  const isLoading = masteryConfigsQuery.isPending
  ...
  return (
    <VersionControl
      ...
      otherData={[]}
      contextID={contextID}
      activeConfigs={masteryConfigsQuery.ocENABLED}
      disabledConfigs={masteryConfigsQuery.ocDISABLED}
      draftConfigs={masteryConfigsQuery.ocDRAFT}
      page="mastery"
      ...
      onDeleteConfig={(configID) => {
        if (!configID) {
          ...
          const shouldDelete = window.confirm('Delete Mastery Config?')
